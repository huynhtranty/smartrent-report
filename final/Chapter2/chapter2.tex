...
Trong kỷ nguyên số hóa, các hệ thống tìm kiếm và giao dịch bất động sản cho thuê trực tuyến đóng vai trò huyết mạch trong việc kết nối nhu cầu xã hội. Phân khúc nhà trọ và phòng cho thuê bình dân chiếm tỷ trọng vô cùng lớn tại các nước đang phát triển, đặc biệt là Việt Nam. Phần này trình bày khái quát các khái niệm cơ bản liên quan đến thị trường cho thuê nhà trọ, cùng các mô hình hệ thống công nghệ đang vận hành phổ biến hiện nay.
...
\begin{enumerate}
    \item \textbf{Mô hình bảng tin rao vặt truyền thống (Classified Ads):} Các website vận hành như một bảng tin công cộng (ví dụ: Phongtro123, các group Facebook). Người cho thuê đăng tin tự do và người thuê tự liên hệ trực tiếp. Ưu điểm là lượng tin phong phú và chi phí đăng thấp. Nhược điểm chí tử là không kiểm soát được chất lượng tin đăng, tỷ lệ tin ảo, tin trùng lặp hoặc lừa đảo chiếm tỷ lệ rất cao, gây lãng phí lớn về thời gian và công sức cho người dùng.
    \item \textbf{Mô hình sàn giao dịch tích hợp dịch vụ (Active Marketplace):} Các nền tảng đứng ra làm trung gian kiểm soát thông tin bài đăng và hỗ trợ các tính năng giá trị gia tăng như bản đồ tìm kiếm, thanh toán hóa đơn trực tuyến, và phân quyền môi giới chuyên nghiệp (ví dụ: Nhà Tốt, Batdongsan.com.vn). Mô hình này cải thiện tính minh bạch nhưng phần lớn quy trình vận hành vẫn chạy bằng nhân lực thủ công (nhân viên duyệt tin, nhân viên tư vấn gọi điện), dẫn đến tốc độ xử lý chậm và khó mở rộng quy mô một cách tối ưu.
    \item \textbf{Mô hình nền tảng thông minh tích hợp trí tuệ nhân tạo (AI-Driven Rental Platform):} Là mô hình thế hệ mới mà đề tài này hướng đến xây dựng (SmartRent). Hệ thống tận dụng sức mạnh của các mô hình ngôn ngữ lớn (LLM) và học máy để tự động hóa hoàn toàn các tác vụ phức tạp: AI Agent hỗ trợ tư vấn và so sánh phòng bằng ngôn ngữ tự nhiên, AI tự động kiểm duyệt hình ảnh và nội dung tin đăng để lọc tin rác, AI định giá dựa trên ngữ cảnh thị trường, và gợi ý tin cá nhân hóa theo hành vi thời gian thực. Mô hình này giúp tối đa hóa trải nghiệm người dùng, giảm thiểu hơn 80\% chi phí vận hành thủ công và nâng cao độ an toàn cho giao dịch.
\end{enumerate}
...
Để giải quyết triệt để các hạn chế của phương pháp truyền thống, đề tài đã nghiên cứu và ứng dụng một chuỗi các công nghệ trí tuệ nhân tạo (AI) tiên tiến nhất hiện nay, bao gồm các mô hình ngôn ngữ lớn (LLM), kỹ thuật tạo sinh kết hợp truy xuất (RAG), tác nhân AI (AI Agent) và các mô hình gợi ý cá nhân hóa.

\subsection{Mô hình ngôn ngữ lớn (LLM) và Kỹ thuật Prompting}

Mô hình ngôn ngữ lớn (Large Language Model - LLM) là các mạng nơ-ron tích chập sâu (Deep Neural Networks) dựa trên kiến trúc Transformer, được huấn luyện trên các tập dữ liệu văn bản khổng lồ để hiểu và tạo sinh ngôn ngữ tự nhiên \cite{VaswaniTransformer}. Trong đề tài này, mô hình Gemini của Google được sử dụng làm nền tảng trí tuệ cốt lõi nhờ khả năng xử lý đa phương tiện vượt trội và hiệu năng suy luận xuất sắc.

Trong quá trình tích hợp LLM, kỹ thuật thiết kế chỉ thị (Prompt Engineering) đóng vai trò quyết định để định hình hành vi phản hồi của mô hình. Nhóm nghiên cứu đã ứng dụng hai kỹ thuật chính:
\begin{itemize}
    \item \textbf{In-Context Learning (Học ngữ cảnh / Few-Shot Prompting):} Thay vì tinh chỉnh (fine-tune) lại trọng số mô hình vốn tốn kém tài nguyên, hệ thống cung cấp trực tiếp các ví dụ mẫu (inputs-outputs) hoặc các dữ liệu thực tế lân cận ngay trong prompt. Điều này giúp mô hình nhận diện cấu trúc, phong cách văn phong và quy luật định giá để phản hồi chính xác \cite{Brown2020GPT3}.
    \item \textbf{System Prompting:} Thiết lập một tập quy tắc hành xử cố định cho mô hình (ví dụ: yêu cầu đóng vai làm một trợ lý tìm phòng thân thiện, trung thực, chỉ sử dụng thông tin được cung cấp, không tự vẽ thêm số điện thoại hoặc liên kết ngoài).
\end{itemize}

\subsection{Tạo sinh kết hợp truy xuất (RAG)}

Mô hình ngôn ngữ lớn tuy rất thông minh nhưng lại mắc phải hai hạn chế nghiêm trọng: tri thức tĩnh (chỉ giới hạn trong thời điểm dữ liệu huấn luyện) và hiện tượng ``ảo tưởng'' (tự tạo ra thông tin sai lệch). Nhằm khắc phục hạn chế này, kỹ thuật tạo sinh kết hợp truy xuất (Retrieval-Augmented Generation - RAG) đã được phát triển \cite{Lewis2020RAG}.
...
\begin{enumerate}
    ...
    \item \textbf{Tạo sinh (Generate):} LLM đọc tài liệu này để tổng hợp câu trả lời chính xác, giúp loại bỏ hoàn toàn hiện tượng đoán mò và đảm bảo câu trả lời luôn khớp với quy định pháp luật hoặc thông tin thực tế của hệ thống.
\end{enumerate}

\subsection{Tác nhân trí tuệ nhân tạo (AI Agent) và Vòng lặp ReAct}

Tác nhân AI (AI Agent) là một thực thể phần mềm thông minh có khả năng tự động hóa quy trình ra quyết định để thực hiện một mục tiêu cụ thể bằng cách quan sát môi trường và kích hoạt các công cụ hành động (tools) có sẵn. Trong đề tài, trợ lý chatbot tìm phòng được xây dựng dưới dạng một Agent hoạt động theo vòng lặp ReAct (Reasoning and Acting) \cite{ReActPaper}.
...
\begin{itemize}
    \item \textbf{Suy nghĩ (Thought):} Agent phân tích câu lệnh của người dùng để xác định bước tiếp theo (ví dụ: người dùng yêu cầu ``tìm phòng gần quận 1 dưới 5 triệu'', Agent suy nghĩ: ``Tôi cần gọi công cụ tìm kiếm phòng và lọc theo khu vực quận 1, mức giá tối đa 5.000.000 VND'').
    \item \textbf{Hành động (Action):} Agent quyết định gọi một công cụ cụ thể với tham số đầu vào được trích xuất từ suy nghĩ (ví dụ: gọi công cụ \texttt{search\_listings} với tham số \texttt{district="Quận 1"}, \texttt{max\_price=5000000}).
    \item \textbf{Quan sát (Observation):} Hệ thống thực thi công cụ trong cơ sở dữ liệu và trả về kết quả thô cho Agent quan sát (ví dụ: danh sách 3 phòng trọ khớp yêu cầu).
    \item \textbf{Lặp lại / Trả lời (Response):} Agent đánh giá kết quả quan sát. Nếu đã đủ thông tin, Agent sẽ sinh câu trả lời tự nhiên để phản hồi cho người dùng; nếu chưa đủ, Agent tiếp tục lập luận để thực hiện một hành động khác (ví dụ: gọi thêm công cụ lấy chi tiết của phòng trọ nổi bật nhất). Để tránh vòng lặp vô hạn do lỗi suy luận, hệ thống cấu hình giới hạn tối đa 12 bước thực thi cho mỗi câu hỏi.
\end{itemize}

\subsection{Hệ thống gợi ý lai (Hybrid Recommendation System)}

Hệ thống gợi ý tin đăng giúp cá nhân hóa trang chủ của từng người dùng dựa trên sở thích và hành vi của họ. Hệ thống SmartRent triển khai cơ chế gợi ý lai kết hợp hai hướng tiếp cận chính:
\begin{itemize}
    \item \textbf{Lọc dựa trên nội dung (Content-Based Filtering - CBF):} Gợi ý các tin đăng có đặc trưng tương đồng (tiện ích, khoảng giá, vị trí) với các tin đăng mà người dùng đó đã từng xem, lưu hoặc tương tác nhiều trong lịch sử \cite{LopsCBF}. Độ tương đồng giữa các đặc trưng được tính toán bằng chỉ số độ tương đồng Cosine (Cosine Similarity) trên không gian vector.
    \item \textbf{Lọc cộng tác (Collaborative Filtering - CF):} Tìm kiếm các người dùng có hành vi tương tự (lịch sử xem tin giống nhau) để gợi ý chéo các tin đăng mà người dùng tương tự kia đã thích nhưng người dùng hiện hành chưa biết tới \cite{SarwarCF}.
\end{itemize}
Hai thuật toán này được tích hợp song song và chuẩn hóa điểm số, kết hợp với các hệ số hiệu chỉnh đặc thù (như khoảng cách địa lý từ cơ sở làm việc/học tập của người dùng và mức độ ưu tiên đẩy tin của tài khoản VIP) để xếp hạng danh sách gợi ý cuối cùng đạt độ chính xác cao nhất.
...
Chương này đã trình bày bức tranh tổng quan về thị trường cho thuê nhà trọ trực tuyến, khảo sát chi tiết các ưu nhược điểm của các giải pháp công nghệ hiện hành, đồng thời thiết lập cơ sở lý thuyết vững chắc về trí tuệ nhân tạo (LLM, RAG, AI Agent, hệ thống gợi ý). Qua đó, nhóm nghiên cứu đã chỉ ra khoảng trống công nghệ lớn trên thị trường và chứng minh tính cấp thiết của việc xây dựng hệ thống SmartRent. Chương tiếp theo sẽ tập trung phân tích nghiệp vụ, đặc tả yêu cầu chi tiết để làm cơ sở thiết kế và hiện thực hóa hệ thống.
